...
| `single_decode_with_kv_cache` | `SINGLE_DECODE` + `SingleAttentionMetadata` | Public facade + Host reference + ephemeral mode-bound provider runtime |
...
| `run()` | active plan + private executor | single 与三类 batch wrapper 均有同名 Host facade；三类 batch wrapper 可进入私有 provider runtime |
...
Single prefill/decode 是 functional API，不要求用户持有 wrapper。NPU tensor-like 输入会在
一次函数调用内完成 registry snapshot、canonical plan、provider selection 和 run；该临时
runtime 不会泄漏到返回值或后续调用。Single prefill 的 ReferenceTensor 要求显式
`backend="reference"`；single decode 因上游无 backend 参数，以 ReferenceTensor 本身作为
Host opt-in。
